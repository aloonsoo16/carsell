...
export default function NewVehicle({ vehicle }) {
  ...
  // Recorremos las opciones fuera del useMemo

  const generateCommandContent = (options, name) => {
    return (
      <Command>
        <CommandInput
          placeholder={`${
            formFields.find((field) => field.name === name)?.placeholder ||
            "Selecciona el equipamiento adicional"
          }...`}
          className="h-9"
        />
        <CommandList>
          ...
          <CommandGroup>
            {options.map((option) => (
              <MemoizedCommandItem
                ...
                label={option}
                isSelected={selectedFeatures.includes(option)}
              />
            ))}
          </CommandGroup>
        </CommandList>
        ...
        <CardContent>
          <div className="flex flex-col md:grid md:grid-flow-row-dense md:grid-cols-3 gap-4">
            {formFields.map(({ name, label, type, options }) => {
              ...
              if (name === "features") {
                // Aquí llamamos a generateCommandContent para crear el contenido de Command
