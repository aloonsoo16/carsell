...
export default function VehicleFilter() {
  ...
  useEffect(() => {
        ...
        const filtered = vehicles.filter((vehicle) => {
          ...
          const brandModelMatches =
            ...
            (brandModelSelection.brands.includes(vehicle.brand) &&
              ...
              brandModelSelection.versions.includes(vehicle.version));
    ...
    if (brandModelSelection.versions.length > 0)
      params.set("versions", brandModelSelection.versions.join(","));
  ...
  const handleClearFilters = () => {
    setFuelTypes([]);
    setBrandModelSelection({ brands: [], models: [], versions: [] });
    setBodyTypes([]);
    setMinYear("");
    setMaxYear("");
    setMinPower("");
    ...
    switch (filterType) {
      ...
      case "brand":
        setBrandModelSelection((prev) => {
          ...
          const versionsToRemove = modelsToRemove.flatMap((model) =>
            Object.keys(brandModelOptions[value].models[model]?.versions || {})
          );
          ...
          return {
            ...
            versions: prev.versions.filter(
              (version) => !versionsToRemove.includes(version)
            ),
          };
        });
        break;
      case "model":
        setBrandModelSelection((prev) => {
          ...
          // Encontrar la marca asociada al modelo eliminado
          const brand = Object.keys(brandModelOptions).find((brand) =>
            Object.keys(brandModelOptions[brand].models).includes(value)
          );

          // Si se encuentra la marca, obtener las versiones asociadas al modelo eliminado
          const associatedVersions =
            brandModelOptions[brand]?.models[value]?.versions || {};

          // Filtrar las versiones que no están asociadas al modelo eliminado
          const newVersions = prev.versions.filter(
            (version) => !Object.keys(associatedVersions).includes(version)
          );
          ...
          // Devolver el nuevo estado, actualizando marcas, modelos y versiones
          return {
            brands: newBrands,
            models: newModels,
  ...
  vehicles.forEach((vehicle) => {
    if (!brandModelOptions[vehicle.brand]) {
      brandModelOptions[vehicle.brand] = {
        ...
        models: {
          [vehicle.model]: {
            ...
            versions: {
              [vehicle.version]: {}, // Eliminar el recuento de las versiones
            },
          },
        },
      };
      ...
      if (!brandModelOptions[vehicle.brand].models[vehicle.model]) {
        ...
        if (
          !brandModelOptions[vehicle.brand].models[vehicle.model].versions[
            vehicle.version
          ]
        ) {
          brandModelOptions[vehicle.brand].models[vehicle.model].versions[
            vehicle.version
          ] = {}; // Sin mostrar el recuento
        }
      }
    }
  });
            ...
            <div className="md:w-full w-80">
              ...
              <div className="w-full flex flex-wrap-reverse md:flex-wrap scroll gap-x-2 gap-y-2 py-4">
                ...
                {Object.keys(brandModelOptions).map((brand) => {
                  ...
                  const allVersionsSelected = Object.keys(
                    brandModelOptions[brand].models
                  ).every((model) => {
                    const versions =
                      brandModelOptions[brand].models[model]?.versions || {};
                    return Object.keys(versions).every((version) =>
                      brandModelSelection.versions.includes(version)
                    );
                  });
                  ...
                  return brandModelSelection.models
                    .filter((model) =>
                      Object.keys(brandModelOptions[brand].models).includes(
                        model
                      )
                    )
                    .map((model) => {
                      ...
                      const selectedVersions = Object.keys(versions).filter(
                        (version) =>
                          brandModelSelection.versions.includes(version)
                      );
