...
export function BrandModelCombobox({
  ...
  const handleSelect = (type, brand, model, version) => {
    ...
    if (type === "model") {
      // Si el modelo está seleccionado, quitamos la selección
      if (newSelectedValues.models.includes(model)) {
        newSelectedValues.models = newSelectedValues.models.filter(
          (m) => m !== model
        );
        // También quitamos la selección las versiones de ese modelo
        newSelectedValues.versions = newSelectedValues.versions.filter(
    ...
    if (type === "version") {
      ...
      // Si ya no quedan versiones para esta marca, eliminamos la marca
      if (
  ...
  const getDisplayText = () => {
    ...
    // Si no hay marcas, modelos ni versiones seleccionadas, mostrar el placeholder original
    ...
    const displayText = brands
      .map((brand) => {
        ...
        if (!brandData) {
          return null; // Si no existe la marca, retornamos null
        }
        ...
        // Analizamos las versiones seleccionadas en base a el modelo 
        const modelTexts = brandModelKeys
          ...
          .map((model) => {
            ...
            // Filtrar versiones seleccionadas para ese modelo seleccionado
      ...
      .join(" | "); // Combinar las marcas con "; " si hay varias marcas seleccionadas

    // Si no hay texto generado, muestra el placeholder original
  ...
  const CommandContent = () => (
    <Command>
      ...
      <CommandList>
        ...
        <CommandGroup className="hover:cursor-not-allowed">
